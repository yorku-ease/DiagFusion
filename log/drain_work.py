...
def drain_work(input_root_dir, file_size=128 * 1024 * 1024):
    ...
    log_format = "<datetime>,<service>,<Content>"
    regex = [
        r"\b[a-zA-Z]{2,3}service[1-2]\b",  # service name
        r"\b[a-fA-F0-9]{16}\b",  # traceID
    ...
    for root, dirs, files in inputs:
        for file in files:
            if file.endswith("csv"):
                ...
                input_dir = (
                    root if size <= file_size else f"mid/{root[5:]}/{file[:-4]}/splits"
                )
                ...
                if size <= file_size:
                    log_file = file
                    if (
                        f"{log_file}_structured.csv" in exists
                        and f"{log_file}_templates.csv" in exists
                    ):
                    ...
                    for log_file in files:
                        ...
                        print(f"[parse]{log_file}")
                        print(
                            f"[parse] input_dir: {input_dir}  output_dir: {output_dir}"
                        )
                        parser.parse(log_file)
                        print(f"[parse done]{log_file}")
