...
#             config['MIN_TEST_LENGTH'] = 5
    ...
    def detection(self, data: cudf.DataFrame, column: str, start_ts: int, end_ts: int):
        """
        return
        ...
        test_data = data[(data['timestamp'] >= start_ts) & (data['timestamp'] < end_ts)]
        ...
        # Use CuPy arrays instead of NumPy arrays
        values = cp.asarray(test_data[column].values)
        timestamps = cp.asarray(test_data['timestamp'].values)
        win_values = cp.asarray(values[:win_size])
        
        mean = cp.mean(win_values)
        std = cp.maximum(cp.std(win_values), std_thr * mean)
        ...
        for i in range(win_size, len(values)):
            if abs(values[i] - mean) > k_thr * std:
                ...
                return True, timestamps[i].item(), anomaly_score
            
            # Update the sliding window
            win_values = cp.append(win_values[1:], values[i])
            mean = cp.mean(win_values)
            std = cp.maximum(cp.std(win_values), std_thr * mean)
