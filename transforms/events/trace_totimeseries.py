...
class TraceUtils:
    ...
    def getPairs(self):
        services_pairs = {
            "webservice": ["mobservice", "redisservice"],
            "mobservice": ["redisservice"],
            "logservice": ["dbservice", "redisservice"],
    ...
    def get_trace_by_day(self, day: int):
        ...
        for service in os.listdir(self.data_dir):
            filepath = os.path.join(
                self.data_dir, f"{service}/trace_{service}_2021-07-{day}.csv"
            )
    ...
    def turn_to_timeseries(self, day, savepath):
        ...
        for caller, callee in self.pairs:
            print(caller, callee)
            temp = df.loc[
                (df["service_name"] == caller) & (df["cservice_name"] == callee)
            ]
            info = {"timestamp": ts, "200": [], "500": [], "other": [], "latency": []}
            for k in range(points_count):
                chosen = temp.loc[
                    (temp["timestamp"] >= start_ts + k * delta)
                    & (temp["timestamp"] < start_ts + (k + 1) * delta)
                ]
                cur_latency = max(0, np.mean(chosen["latency"].values))
                ...
                for callee in services_pairs[caller]:
                    for i in [1, 2]:
                        for j in [1, 2]:
                            pairs.append((caller + str(i), callee + str(j)))
            pairs.extend(
                [("logservice1", "logservice2"), ("logservice2", "logservice1")]
            )
    ...
    def get_invocation_events(self):
        ...
        if self.dataset == "gaia":
            for case_id, case in tqdm(self.cases.iterrows()):
                day = int(case["st_time"].split(" ")[0].split("-")[-1])
                index = case["index"]
                ...
                for caller, callee in self.pairs:
                    ...
                    res1 = self.detector.detection(
                        invocation_data, "latency", start_ts, end_ts
                    )
                    res2 = self.detector.detection(
                        invocation_data, "500", start_ts, end_ts
                    )
                    #                 print(res1, res2)
                    ...
                    if res2[0]:
                        ...
                        if ts == res2[1]:
                            score = res2[2]
                    self.res[index].append((int(ts), caller, callee, score))
...
if __name__ == "__main__":
    with open("config/gaia_config.yaml", "r", encoding="utf-8") as f:
        ...
        trace_data_dir = "data/gaia/demo/demo_1100/processed_traces"
        labels = pd.read_csv(label_path)
        invocation_event = InvocationEvent(
            labels, trace_data_dir, config["dataset"], None
        )
        invocation_event.get_invocation_events()

        save_path = "data/gaia/test/anomalies/trace.json"
